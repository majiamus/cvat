...
export class AutoborderHandlerImpl implements AutoborderHandler {
    ...
    private drawMarkers(transformedShapes: TransformedShape[]): void {
        ...
        // 为每个形状创建一个组，包含所有控制点
        this.groups = transformedShapes.map((shape: TransformedShape, groupID: number): SVGGElement => {
            // 创建SVG组元素
                    ...
                    const click = (event: MouseEvent): void => {
                        ...
                        if (this.auxiliaryClicks.length === 1) {
                            ...
                            for (const wayPoint of way) {
                                const [pX, pY] = wayPoint.split(',').map((coordinate: string): number => +coordinate);
    ...
    public updateObjects(): void {
        ...
        const shapes = Array.from(this.frameContent.getElementsByClassName('cvat_canvas_shape')).filter(
            (shape: HTMLElement): boolean =>
                +shape.getAttribute('clientID') !== this.currentID && !shape.classList.contains('cvat_canvas_hidden'),
        );
        // 将形状转换为变换后的形状对象
