...
export class CanvasViewImpl implements CanvasView, Listener {
    ...
    /**
     * 设置问题区域方法
     * 根据传入的问题区域数据更新画布上显示的问题区域
     * @param issueRegions - 问题区域对象，键为区域ID，值为包含隐藏状态和点坐标的对象
     */
    ...
    public constructor(model: CanvasModel & Master, controller: CanvasController) {
        ...
        this.bitmap.style.display = 'none';  // 位图默认隐藏
        ...
        this.canvas.appendChild(this.bitmap);       // 位图层
        this.canvas.appendChild(this.grid);         // 网格层
    ...
    /**
     * 设置冲突区域的位置
     * @param {any} state - 对象状态，包含clientID等属性
     * @returns {number[]} 冲突区域的中心坐标[cx, cy]，如果图形不存在则返回空数组
     */
    ...
    /**
     * 重绘位图，用于在画布上显示对象
     * 将所有可见对象绘制到黑色背景的位图上，对象以白色显示
     */
    private redrawBitmap(): void {
        // 增加位图更新请求ID，用于防止异步操作冲突
            ...
            for (const state of states) {
                ...
                if (['rectangle', 'polygon', 'cuboid'].includes(state.shapeType)) {
                    ...
                    if (state.shapeType === 'rectangle') {
                        // 计算矩形的中心点
                        const centerX = points[0] + (points[2] - points[0]) / 2;
                        const centerY = points[1] + (points[3] - points[1]) / 2;
                        // 旋转矩形的四个角点
                ...
                if (state.shapeType === 'ellipse') {
                    ...
                    // 计算椭圆的半径
                    const radiusX = rightX - cx;
                    const radiusY = cy - topY;
                    // 将旋转角度转换为弧度
                    const rotation = (state.rotation * Math.PI) / 180.0;
                    
                    // 绘制椭圆
                    ctx.beginPath();
                    ctx.ellipse(cx, cy, radiusX, radiusY, rotation, 0, 2 * Math.PI);
                    ctx.closePath();
                    ctx.fill();
                }
                ...
                if (state.shapeType === 'cuboid') {
                    ...
                    for (let i = 0; i < 5; i++) {
                        // 计算每个面的点
                        const points = [
                            state.points[(0 + i * 4) % 16], // 点0 x
                            state.points[(1 + i * 4) % 16], // 点0 y
                            state.points[(2 + i * 4) % 16], // 点1 x
                            state.points[(3 + i * 4) % 16], // 点1 y
                            state.points[(6 + i * 4) % 16], // 点2 x
                            state.points[(7 + i * 4) % 16], // 点2 y
                            state.points[(4 + i * 4) % 16], // 点3 x
                            state.points[(5 + i * 4) % 16], // 点3 y
                        ];
    ...
    /**
     * 添加立方体到画布
     * @param points - 立方体顶点坐标字符串，定义3D立方体的各个顶点
    ...
    private addCuboid(points: string, state: any): any {
        ...
        const cube = (this.adoptedContent as any)
            .cube(points) // 使用自定义的cube方法创建3D立方体
            .fill(state.color) // 设置填充颜色
            .attr({
                clientID: state.clientID,
                'color-rendering': 'optimizeQuality',
                id: `cvat_canvas_shape_${state.clientID}`,
